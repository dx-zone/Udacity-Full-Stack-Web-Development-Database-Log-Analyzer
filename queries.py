...
JOIN (SELECT path, count(*) AS views FROM log  GROUP BY path ORDER BY views) AS newlog
ON replace(newlog.path,'/article/','') = authors_table.slug GROUP BY authors_table.author ORDER BY views DESC;
"""
...
SELECT *
FROM
(SELECT request_table.datedata, cast(round((errTable.error * 100.00)/request_table.request,2) AS numeric) AS err
